...
type DbGiftInsert = Database["public"]["Tables"]["gifts"]["Insert"]
type DbGiftUpdate = Database["public"]["Tables"]["gifts"]["Update"]
...
export async function getGiftById(id: string): Promise<Gift | null> {
  const { data, error } = await supabase.from("gifts").select("*").eq("id", id).single() as { data: DbGift | null; error: any }
...
export async function createGift(giftData: GiftData): Promise<{ success: boolean; error?: string; gift?: Gift }> {
  ...
  const { data, error } = await supabase
    ...
    .select()
    .single() as { data: DbGift | null; error: any }
