...
type DbAttendance = Database["public"]["Tables"]["event_attendance"]["Row"]
...
export async function getGuestAttendance(guestId: string): Promise<AttendanceConfirmation | undefined> {
  try {
    const { data, error } = await supabase
      ...
      .select("*")
      .eq("guest_id", guestId)
      .maybeSingle() as { data: DbAttendance | null; error: any }
...
export async function getAllAttendances(): Promise<AttendanceConfirmation[]> {
  try {
    const { data, error } = await supabase
      .from("event_attendance")
      .select("*")
      .order("confirmed_at", { ascending: false }) as { data: DbAttendance[] | null; error: any }
